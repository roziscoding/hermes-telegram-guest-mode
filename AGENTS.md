...
4. Run `hermes plugins install https://github.com/roziscoding/hermes-telegram-guest-mode --enable`, then `hermes gateway restart`.
5. Verify `hermes gateway status` and normal Telegram connectivity. Guest Mode still has to be enabled separately in BotFather.
6. If startup or Telegram connectivity fails, run `hermes plugins disable telegram-guest` and restart the gateway to restore the bundled adapter.
